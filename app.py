...
class CameraThread(threading.Thread):
    ...
    def __init__(self, camera_id, camera_name, knn, mode='attendance', search_user=None, shared_stop_flag=None):
        ...
        self.found = False
        self.shared_stop_flag = shared_stop_flag  # Shared flag to stop all cameras
    ...
    def run(self):
        ...
        try:
            while not self.stop_flag:
                # Check shared stop flag (for search mode when any camera finds the user)
                if self.shared_stop_flag is not None and self.shared_stop_flag['stop']:
                    print(f"🛑 Stopping {self.camera_name} - User found on another camera")
                    break
                ...
                if not ret:
                    ...
                    if consecutive_failures >= max_failures:
                        print(f"❌ {self.camera_name}: Too many frame read failures, stopping...")
                        break
                    continue
                ...
                # Always show the frame (even if not processing faces this frame)
                ...
                if frame_count % process_every_n_frames == 0:
                    ...
                    if len(faces) > 0:  # Only process if faces detected
                        for (x, y, w, h) in faces:
                            ...
                            if self.mode == 'attendance':
                                ...
                                if identified_person == self.search_user:
                                    ...
                                    # Create SearchLog file if it doesn't exist
                                    search_log_path = f'Attendance/SearchLog-{datetoday}.csv'
                                    ...
                                    # Signal all cameras to stop IMMEDIATELY
                                    if self.shared_stop_flag is not None:
                                        self.shared_stop_flag['stop'] = True
                                    
                                    self.stop_flag = True
                                    print(f"✅ User found on {self.camera_name}! Signaling all cameras to stop...")
                                    
                                    # Draw the detection before breaking
                                    cv2.rectangle(display_frame, (x, y), (x+w, y+h), color, 3)
                                    cv2.putText(display_frame, text, (x, y-10), 
                                               cv2.FONT_HERSHEY_SIMPLEX, 0.8, color, 2)
                                    cv2.putText(display_frame, self.camera_name, (10, 30), 
                                               cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 255), 2)
                                    cv2.imshow(f'{self.mode.title()} - {self.camera_name}', display_frame)
                                    cv2.waitKey(500)  # Show for half second
                                    break  # Exit the for loop
                                else:
                                    color = (100, 100, 100)
                                    text = identified_person if identified_person else "Searching..."
                        ...
                        if self.mode == 'search':
                            cv2.putText(display_frame, "Searching for faces...", (10, 60), 
                                       cv2.FONT_HERSHEY_SIMPLEX, 0.6, (200, 200, 200), 2)
                
                # Show camera name on every frame
                cv2.putText(display_frame, self.camera_name, (10, 30), 
                           cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 255), 2)
                
                cv2.imshow(f'{self.mode.title()} - {self.camera_name}', display_frame)
                
                # Check if we should stop (found user in search mode)
                if self.stop_flag:
                    break
            ...
            print(f"❌ Error in {self.camera_name}: {str(e)}")
        finally:
            cap.release()
            cv2.destroyWindow(f'{self.mode.title()} - {self.camera_name}')
            print(f"📷 {self.camera_name} closed")
...
def search_user():
    ...
    # Shared stop flag - when one camera finds user, all stop
    shared_stop_flag = {'stop': False}
    
    # Start threads for each camera
    threads = []
    for cam_id, cam_name in CAMERAS.items():
        thread = CameraThread(cam_id, cam_name, knn, mode='search', 
                            search_user=searchuser, shared_stop_flag=shared_stop_flag)
        thread.daemon = True  # Make threads daemon so they die with main program
        thread.start()
        threads.append(thread)
    
    # Wait for all threads with timeout checking
    import time
    ...
    while any(t.is_alive() for t in threads):
        time.sleep(0.1)
        # Check if user found - stop waiting
        if shared_stop_flag['stop']:
            print("🎯 User found! Waiting for all cameras to close...")
            time.sleep(1)  # Give time for all threads to see the flag
            break
        # Timeout check
        if time.time() - start_time > timeout:
            print("⏱️ Search timeout reached")
            shared_stop_flag['stop'] = True
            break
